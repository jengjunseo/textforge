...
const COMMANDS = [
  ...
  { id: "diagnostics-full", title: "전체 벤치마크 실행", hint: "더미 문서 100개 memory-only 테스트", run: () => openDiagnosticsPanel().then(() => window.TextForgeDiagnostics?.runFullBenchmark()) },
  { id: "diagnostics-reliability", title: "신뢰성 반복 테스트", hint: "반복 작업 100회 MTBF-like 측정", run: () => openDiagnosticsPanel().then(() => window.TextForgeDiagnostics?.runReliabilityTest()) },
  { id: "diagnostics-snapshot", title: "Forge Snapshot 벤치마크", hint: "Snapshot HTML build 성능 측정", run: () => openDiagnosticsPanel().then(() => window.TextForgeDiagnostics?.runSnapshotBenchmark()) },
  ...
  { id: "diagnostics-mttr", title: "MTTR 복구 테스트", hint: "격리된 장애 주입으로 복구 시간 측정", run: () => openDiagnosticsPanel().then(() => window.TextForgeDiagnostics?.runMttrBenchmark()) },
  { id: "document-switch-benchmark", title: "문서 전환 벤치마크 실행", hint: "문서 클릭 전환 흐름의 단계별 지연 측정", run: () => runDocumentSwitchBenchmark({ samples: 10 }) },
  { id: "document-switch-trace-show", title: "최근 문서 전환 trace 보기", hint: "최근 전환의 단계별 기록을 console에 표시", run: showLastDocumentSwitchTrace },
  { id: "document-switch-trace-export", title: "문서 전환 벤치마크 JSON 내보내기", hint: "최근 문서 전환 측정 결과 저장", run: exportDocumentSwitchBenchmarkJson },
...
function renderCommands() {
  ...
  const items = [...COMMANDS, ...docCommands].filter((item) =>
    `${item.title} ${item.hint}`.toLowerCase().includes(query),
  );
